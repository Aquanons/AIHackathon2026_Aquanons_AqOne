(function () {
  ...
  function syncAlertIndicators() {
    var currentActiveCount = alertData.filter(function (alert) { return alert.status === 'active'; }).length;
    var alertBadge = document.getElementById('badge-alerts');
    if (alertBadge) alertBadge.textContent = currentActiveCount;
    if (bannerCountEl) bannerCountEl.textContent = currentActiveCount;
    if (liveBanner) liveBanner.classList.toggle('has-alerts', currentActiveCount > 0);
    renderAlerts();
  }
  ...
  function updateStats() {
    ...
    var statCatches = document.getElementById('stat-catches');
    var statAlerts = document.getElementById('stat-alerts');
    if (statPopulation) statPopulation.textContent = (54320 + facilitiesInView * 1200).toLocaleString();
    if (statActive) statActive.textContent = (1247 + facilitiesInView * 85).toLocaleString();
    if (statProjects) statProjects.textContent = Math.max(18, facilitiesInView * 3);
    if (statCatches) statCatches.textContent = (3891 + buoysInView * 320).toLocaleString() + ' kg';
    if (statAlerts) statAlerts.textContent = incidentsInView;
  }
